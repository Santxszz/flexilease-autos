...
export default class UserAuthService {
	public async execute({ email, password }: IRequest) {
        ...
        const decodedPassword = await bcrypt.compare(password, findUserByEmail.password)
        if(decodedPassword) {
            const tokenPayload = {
            ...
            const userToken = await jwt.sign(tokenPayload, process.env.JWT_SECRET as string, {expiresIn: '12h'})
            return userToken
        }
